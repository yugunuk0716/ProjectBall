...
public class AStarManager : MonoBehaviour
{
    ...
    public void PathFinding()
    {
        ...
        NodeArray = new Node[(int)sizeX, (int)sizeY];
    ...
    void OpenListAdd(float checkX, float checkY)
    {
        ...
        {
            // 대각선 허용시, 벽 사이로 통과 안됨
            if (allowDiagonal) if (NodeArray[(int)(CurNode.x - bottomLeft.x), (int)(checkY - bottomLeft.y)].isWall && NodeArray[(int)(checkX - bottomLeft.x), (int)(CurNode.y - bottomLeft.y)].isWall) return;

            // 코너를 가로질러 가지 않을시, 이동 중에 수직수평 장애물이 있으면 안됨
            if (dontCrossCorner) if (NodeArray[(int)(CurNode.x - bottomLeft.x), (int)(checkY - bottomLeft.y)].isWall || NodeArray[(int)(checkX - bottomLeft.x), (int)(CurNode.y - bottomLeft.y)].isWall) return;
